...
export default function GraphVisualizer({ graph, highlightedPath, source, destination, gpsLocations }) {
  ...
  useEffect(() => {
    ...
    Object.entries(coordMap).forEach(([loc, coord]) => {
      ...
      // --- Dynamic scaling ---
      const baseDot = 2.5;                  // base radius for all nodes
      const zoomedDot = baseDot * scale;    // dot grows slightly when zooming
      const fontSize = Math.max(10, 11 * scale); // adaptive font scaling
      const labelOffset = 10 * scale;       // space between dot and label
      ...
      else ctx.fillStyle = coord.campus === "North" ? "#667eea" : "#764ba2";
      ctx.fill();
      ...
      Object.entries(coordMap).forEach(([loc, coord]) => {
        ...
        else ctx.fillStyle = coord.campus_type === "North" ? "#667eea" : "#764ba2";
        ctx.fill();
